...
| **osproxy-kafka-wal** | The **durable spill buffer**: a `Producer` that persists each record to a CRC-framed write-ahead log and a background drainer delivers it to an `AckProducer`, advancing an on-disk checkpoint only on broker ack. Survives a restart (at-least-once); broker-agnostic, so it wraps any `AckProducer`. | kafka |
| **osproxy-kafka-rdkafka** | A librdkafka `Producer` binding for strict OpenSSL-FIPS deployments. **Not a workspace member** (root `Cargo.toml` `exclude`): it links a heavy native library, so the gate/CI never build it. Build it on its own on a host with cmake + curl/OpenSSL dev headers. | kafka (path) |
