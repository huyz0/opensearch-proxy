...
| **osproxy-kafka-rskafka** | The **portable** `Producer`: pure-Rust rskafka + rustls, no native build, one client for both deployment targets. The TLS crypto provider is build-selected (ring / FIPS aws-lc-rs) like the rest of osproxy. See the crate's FIPS caveat (rskafka 0.6 links ring). | kafka |
