...
| `forward_client_headers` | `true` | When the proxy forwards a request (the verbatim passthrough/admin/cursor paths) it rebuilds the upstream request; this decides whether the client's **own** headers ride along. Default `true` (sidecar trust): pass every client header through, minus the mandatory hop-by-hop/framing set (`connection`, `transfer-encoding`, `host`, `content-length`, …). Set `false` to send only the proxy-managed headers (content type, trace). |
| `forward_header_deny` | *(unset → nothing denied)* | Comma-separated, case-insensitive list of client headers to **drop** on top of the mandatory set, e.g. `authorization` to keep the client credential off the cluster. Only meaningful with `forward_client_headers=true`. |
